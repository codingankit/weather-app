...
const fetchWeather = async (city) => {
  const apiForFetchWeather = "/api/weather";
...
const evaluateIconClass = (iconInfoArr) => {
...
const evaluateWeather = () => {
  ...
  weatherInfoProm
    .then((weatherInfo) => {
      if (weatherInfo == 0) return noCityFound();
      const city = weatherInfo.name;
      const temp = `${weatherInfo.main.temp} °C`;
      const iconClass = evaluateIconClass(weatherInfo.weather[0]);
    ...
    .catch(() => {
      loaderDiv.style.display = "none";
      errDiv.style.display = "block";
      errText.innerText = "Something went wrong. Please try again.";
    });
};
